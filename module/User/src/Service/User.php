...
class User
{
    public function __construct(
        ...
        private readonly string $pwnedPasswordsHost,
    ) {
    }
    ...
    public function activate(
        ...
        if ($this->isPasswordLeaked($data['password'])) {
    ...
    public function userLogin(array $data): ?UserModel
    {
        ...
        if ($identity instanceof UserModel) {
            if ($this->isPasswordLeaked($data['password'])) {
                ...
                $identity = $this->detachUser($identity);
                $identity->setMustChangePassword(true);

                $this->userMapper->persist($identity);
            }
    ...
    public function companyLogin(array $data): ?CompanyUserModel
    {
        ...
        if ($identity instanceof CompanyUserModel) {
            if ($this->isPasswordLeaked($data['password'])) {
                /** @var CompanyUserModel $identity */
                $identity = $this->detachUser($identity);
                $identity->setMustChangePassword(true);

                $this->companyUserMapper->persist($identity);
            }
    ...
    /**
     * Check a password against a Pwned Passwords instance to determine if it was leaked in a public breach.
     */
     ...
     *
     * Taken from the LoginAttempt service, where this is also an issue.
     *
     * @psalm-param T $user
     * @psalm-return (T is CompanyUserModel ? CompanyUserModel : (T is UserModel ? UserModel : false))
     */
    private function detachUser(IdentityInterface $user): ?IdentityInterface
    {
